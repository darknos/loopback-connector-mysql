...
function buildOrderBy(self, model, order) {
  ...
  //allow using getdistance function in orderby clause like "getdistance(fld1,fld2,54.098989,33.5656565) desc"
  //loopback juggler already splitted it by comma to something like [ 'getdistance(geo_lng', 'geo_lat', '50', '30) DESC' ]
  //join splitted getdistance function call
  var joined = [];
  while (order.length > 0) {
    var o = order.shift();
    if (o.match(/^getdistance\(/)) {//'getdistance(geo_lng'
      o += "," + order.shift();//, 'geo_lat'
      o += "," + order.shift();//, 50.45454
      o += "," + order.shift();//, 30.45454) DESC'
    }
    joined.push(o);
  }
  order = joined;
  ...
  return 'ORDER BY ' + order.map(function (o) {
    //if it starts from getdistance - don't split it by comma again
    if (o.match(/^getdistance\(/)) {
      return o;
    }
    var t = o.split(/[\s,]+/);
    if (t.length === 1) {
